...
CREATE TABLE members (
	...
	exp             VARCHAR(50)  NULL     DEFAULT 0 COMMENT '점수', -- 점수
...
CREATE TABLE study_comments (
	...
	order        INT          NOT NULL DEFAULT 1 COMMENT '순서', -- 순서
	created_date DATETIME     NULL     DEFAULT now() COMMENT '생성일', -- 생성일
...
ALTER TABLE common_code_group
	ADD CONSTRAINT PK_common_code_group -- 새 테이블 기본키
	PRIMARY KEY (
	code_group -- 그룹키
	);
...
-- 공통코드
ALTER TABLE common_codes
	ADD CONSTRAINT FK_common_code_group_TO_common_codes -- 새 테이블 -> 공통코드
	FOREIGN KEY (
	code_group -- 그룹키
	)
	REFERENCES common_code_group ( -- 새 테이블
	code_group -- 그룹키
	);
